...
*/
                var length = this.astroidGroup.getLength();
                var greatest = 0;
                this.offset = 30;
                ...
                if (this.astroidGroup.getLength() < 5) {
                    this.astroidGroup.getChildren().forEach(function(sprite) {
                        if (greatest == 0 || sprite.x > greatest) greatest = sprite.x;
                    });
                    this.makeAstroidFunc(game.config.width + 55 + greatest / 10, Phaser.Math.Between(150, game.config.height / 1.5));

                    this.astroidGroup.getChildren().forEach(function(sprite) {
                        if (!avoidGroundSpawn(sprite)) sprite.y = Phaser.Math.Between(150, game.config.height - game.config.height / 2.5);
                    });
                }

                function avoidGroundSpawn(sprite) {
                    if (sprite.y <= game.config.height - game.config.height / 2.5) return true; //sprite.y = Phaser.Math.Between(150, game.config.height - game.config.height / 2.5);
                    return false;
                }
